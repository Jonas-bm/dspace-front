...
export class ThumbnailComponent implements OnChanges {
  /**
   * The DSpace item object to retrieve PDF from
   */
  @Input() item?: any;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (isPlatformBrowser(this.platformID)) {
      ...
      const isThumbnailLoading = this.thumbnail instanceof RemoteData && this.thumbnail.isLoading;
      const hasThumbnailPayload = this.thumbnail instanceof Bitstream || (this.thumbnail instanceof RemoteData && hasValue(this.thumbnail.payload));
      ...
      if (!hasThumbnailPayload) {
        if (this.item) {
          ...
          if (bitstreamService && typeof bitstreamService.findAllByItemAndBundleName === 'function' && this.item._links?.bundles?.href) {
