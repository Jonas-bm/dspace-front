...
export class UsageReportDataService extends IdentifiableDataService<UsageReport> implements SearchData<UsageReport> {
  ...
  searchStatistics(uri: string, page: number, size: number, useCachedVersionIfAvailable: boolean = false): Observable<UsageReport[]> {
    ...
    }, useCachedVersionIfAvailable, false).pipe(
      getFirstSucceededRemoteData(),
      getRemoteDataPayload(),
      map((list) => list.page),
    );
